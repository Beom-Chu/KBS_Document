...
public class StreamTest {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(streamTest.allMatchNumber(liInt, 6));
    System.out.println(streamTest.anyMatchNumber(liInt, 6));

    System.out.println(streamTest.reduceMultiply(liInt));
  ...
  /**
   * 배열값이 모두 up 이상이면 true
   * 
   * @param liInt
   * @param up
   * @return boolean
   */
  public boolean allMatchNumber(List<Integer> liInt, int up) {
    return liInt.stream().allMatch(i -> i > up);
  }

  /**
   * 배열값이 하나라도 up 이상이면 true
   * 
   * @param liInt
   * @param up
   * @return boolean
   */
  public boolean anyMatchNumber(List<Integer> liInt, int up) {
    return liInt.stream().anyMatch(i -> i > up);
  }

  /**
   * Integer List Stream 의 요소를 하나씩 줄여가며 곱셈.
   * 
   * @param li
   * @return int
   */
  public int reduceMultiply(List<Integer> li) {
    return li.stream().reduce(1, (x, y) -> x * y);
  }
